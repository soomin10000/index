...
def glowline(ax, x, y, color, lw=2):
    """Plot a line with a soft glow halo."""
...
def fmt_legend(ax, names, colors):
    handles = [
        plt.Line2D([0], [0], color=c, linewidth=2, label=n,
                   path_effects=[pe.Stroke(linewidth=4, foreground=c, alpha=0.2),
                                 pe.Normal()])
...
DB  = Path.home() / "unifi_poller.db"
OUT = Path(__file__).parent / "dashboard.png"
...
if not weak_rows and not cong_rows:
    print("No data yet — run the poller for a while first.")
    raise SystemExit
...
for ax, panel in zip(axes, panels):
    ...
    if panel == "signal":
        ...
        for name, color in zip(names, colors):
            d = clients[name]
            glowline(ax, d["ts"], d["signal"], color)
            shade(ax, d["ts"], d["signal"], color, invert=True)
            glowdots(ax, d["ts"], d["signal"], color)
        threshold_line(ax, -70, "−70 dBm weak threshold", invert=True)
        ax.invert_yaxis()
        ax.yaxis.set_major_formatter(mticker.FormatStrFormatter("%d dBm"))
        fmt_legend(ax, names, colors)
        ...
        for name, color in zip(names, colors):
            d = clients[name]
            glowline(ax, d["ts"], d["retry_pct"], color)
            shade(ax, d["ts"], d["retry_pct"], color)
            glowdots(ax, d["ts"], d["retry_pct"], color)
        threshold_line(ax, 10, "10% retry threshold")
        ...
        # Coloured zone bands: safe / warning / danger
        xmin, xmax = mdates.date2num(min(
            d["ts"][0] for d in radios.values()
        )), mdates.date2num(max(
            d["ts"][-1] for d in radios.values()
        ))
        ...
        for name, color in zip(names, colors):
            ...
            # Rolling average (window = min(5, n))
            arr = np.array(vals, dtype=float)
            w   = min(7, len(arr))
            if w > 1:
                kernel = np.ones(w) / w
                smooth = np.convolve(arr, kernel, mode="same")
                # Convolve pads edges — trim to valid interior
                smooth[:w//2]  = arr[:w//2]
                smooth[-w//2:] = arr[-w//2:]
            else:
                smooth = arr

            glowline(ax, ts, smooth, color, lw=2)
            shade(ax, ts, smooth, color)
            glowdots(ax, ts, smooth, color)
        ...
        glowline(ax, speed_ts, speed_dl, PALETTE[0], lw=2)
        shade(ax, speed_ts, speed_dl, PALETTE[0])
        glowdots(ax, speed_ts, speed_dl, PALETTE[0])

        glowline(ax, speed_ts, speed_ul, PALETTE[1], lw=2)
        shade(ax, speed_ts, speed_ul, PALETTE[1])
        glowdots(ax, speed_ts, speed_ul, PALETTE[1])
        ...
        fmt_legend(ax, ["Download", "Upload", "Ping"], [PALETTE[0], PALETTE[1], PALETTE[2]])
...
print(f"Saved {OUT} ({len(weak_rows)} weak-client records, {len(cong_rows)} congestion records)")
