"""Writes weeny.json for the web dashboard — CPU/mem/disk/uptime for weeny (the
Pi Zero W running the OpenCanary honeypot), gathered over SSH.

This is the host-health card (load/mem/disk/temp/throttling), distinct from
pollers/honeypot.py which reads weeny's OpenCanary log for the deception-band
/honeypot card — same box, two different things being watched on it.

Metrics history goes to weeny_history.db (5-column schema, with cpu_temp).
"""
...
OUT        = DATA / "weeny.json"
STATE_FILE = DATA / "weeny_state.json"
DB_FILE    = DATA / "weeny_history.db"
...
def _extra_alerts(data):
    """weeny-specific alert candidates: SoC temp, throttle flags, failed units."""
    extra = {}
...
def fetch_and_write():
    ts = int(time.time())
    try:
        s = hostlib.fetch_remote("weeny", REMOTE_SCRIPT)
        ...
        model = s["MODEL"].strip() or "Raspberry Pi Zero W"
